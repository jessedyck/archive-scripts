...
**bsdtar's sparse-file detection is disabled via `--no-read-sparse`** — bsdtar (macOS's default `tar`) probes every file with `lseek(fd, 0, SEEK_HOLE)` before archiving it, to skip sparse holes. On a network mount, or on a physically degrading drive (this tool's whole reason for existing), that probe can hang and fail with `lseek(SEEK_HOLE) failed: Operation timed out` partway through a multi-hour archive run, aborting the entire tar/zstd pipeline with nothing salvaged. `--no-read-sparse` skips the probe and reads files as plain sequential data; any real holes just become long runs of zero bytes, which zstd compresses away almost entirely, so the practical size cost is negligible. Applied unconditionally when `tar --version` identifies as bsdtar — detected at runtime rather than assumed from the OS, since Linux users occasionally have bsdtar installed too. GNU tar (Linux default) has no equivalent flag and never does this probe unless `--sparse` is explicitly passed, so it's left untouched.
